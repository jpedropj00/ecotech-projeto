...
async function buscar() {
    ...
    try {
        ...
        document.getElementById("btnPdf").disabled =
...
function renderCards(dados) {
    ...
    const amp = max - min;
    const t5 =
        vals.slice(-5).reduce((a, b) => a + b, 0) / Math.min(5, vals.length);
    const t10 = vals.slice(-10, -5).length
        ? vals.slice(-10, -5).reduce((a, b) => a + b, 0) /
        vals.slice(-10, -5).length
        : t5;
    const tend =
        t5 > t10 + 1 ? "↑ Subindo" : t5 < t10 - 1 ? "↓ Caindo" : "→ Estável";
...
function renderGrafico(dados) {
    ...
    const yOf = (v) =>
        pad.top + (1 - (v - vMin) / Math.max(1, vMax - vMin)) * cH;
...
function gerarPDF() {
    ...
    setTimeout(() => {
        try {
            ...
            const atual = vals[vals.length - 1];
            const t5 =
                vals.slice(-5).reduce((a, b) => a + b, 0) /
                Math.min(5, vals.length);
            const t10 = vals.slice(-10, -5).length
                ? vals.slice(-10, -5).reduce((a, b) => a + b, 0) /
                vals.slice(-10, -5).length
                : t5;
            const tend =
                t5 > t10 + 1 ? "Subindo" : t5 < t10 - 1 ? "Caindo" : "Estavel";

            doc.setFillColor(11, 18, 32);
            doc.rect(0, 0, W, H, "F");
            doc.setFillColor(19, 30, 53);
            doc.rect(0, 0, W, 22, "F");
            ...
            doc.setTextColor(226, 234, 246);
            doc.setFontSize(11);
            doc.text("Relatorio de Umidade — Dashboard", 38, 14);
            doc.setTextColor(122, 143, 171);
            doc.setFontSize(7.5);
            doc.text(
                `Gerado em: ${agora}   |   ${vals.length} registros`,
                W - 12,
                14,
                { align: "right" },
            );
            ...
            doc.text("ESTATISTICAS GERAIS", 12, 32);
            doc.setDrawColor(36, 48, 80);
            doc.setLineWidth(0.3);
            doc.line(12, 33.5, W - 12, 33.5);

            const statsData = [
                {
                    label: "ATUAL",
                    value: atual.toFixed(1) + "%",
                    cor: [74, 222, 128],
                    sub: "Ultima leitura",
                },
                {
                    ...
                    cor: [56, 189, 248],
                    sub: `${vals.length} amostras`,
                },
                {
                    label: "MAXIMA",
                    value: maxi.toFixed(1) + "%",
                    cor: [244, 114, 182],
                    sub: "Pico registrado",
                },
                {
                    label: "MINIMA",
                    value: mini.toFixed(1) + "%",
                    cor: [250, 204, 21],
                    sub: "Menor registro",
                },
                {
            ...
            statsData.forEach((s, i) => {
                ...
                const cx = 12 + col * (cW2 + 5);
                const cy = 37 + row * (cH2 + 4);
                doc.setFillColor(26, 40, 68);
                doc.setDrawColor(36, 48, 80);
                doc.setLineWidth(0.3);
            ...
            doc.setFontSize(7);
            doc.setFont("helvetica", "normal");
            doc.text("HISTORICO DE UMIDADE", 12, 100);
            doc.setDrawColor(36, 48, 80);
            doc.line(12, 101.5, W - 12, 101.5);
            drawChartPDF(
                doc,
                vals.slice(-60),
                dadosGlobais.slice(-60).map((d) => d.data_hora.slice(11, 16)),
                12,
                104,
                W - 24,
                68,
            ...
            doc.setFontSize(7);
            doc.setFont("helvetica", "normal");
            doc.text("ULTIMAS LEITURAS", 12, 178);
            doc.setDrawColor(36, 48, 80);
            doc.line(12, 179.5, W - 12, 179.5);

            const slice15 = [...dadosGlobais].reverse().slice(0, 15);
            const rows15 = slice15.map((r, i) => {
                const v = parseFloat(r.umidade);
                const st =
                    v >= media + 5 ? "Alto" : v <= media - 5 ? "Baixo" : "Normal";
                return [i + 1, r.data_hora, v.toFixed(1) + "%", st];
            });
            doc.autoTable({
                startY: 181,
                head: [["#", "Data / Hora", "Umidade", "Status"]],
                body: rows15,
                theme: "plain",
                styles: {
                    fontSize: 7.5,
                    textColor: [200, 210, 226],
                    fillColor: [11, 18, 32],
                    cellPadding: 2.2,
                    lineColor: [36, 48, 80],
                    lineWidth: 0.2,
            ...
            doc.setTextColor(226, 234, 246);
            doc.setFontSize(11);
            doc.text("Grafico Completo + Analise", 38, 14);
            doc.setTextColor(122, 143, 171);
            doc.setFontSize(7.5);
            doc.text(`Pagina 2   |   ${agora}`, W - 12, 14, { align: "right" });
            doc.setTextColor(122, 143, 171);
            doc.setFontSize(7);
            doc.setFont("helvetica", "normal");
            doc.text("SERIE COMPLETA DE LEITURAS", 12, 32);
            doc.setDrawColor(36, 48, 80);
            doc.line(12, 33.5, W - 12, 33.5);

            const allLabels = dadosGlobais.map((d) =>
                d.data_hora.slice(11, 16),
            );
            drawChartPDF(doc, vals, allLabels, 12, 36, W - 24, 90);

            doc.setTextColor(122, 143, 171);
            doc.setFontSize(7);
            doc.text("ANALISE DOS DADOS", 12, 134);
            doc.setDrawColor(36, 48, 80);
            doc.line(12, 135.5, W - 12, 135.5);

            const analise = buildAnalise(
                vals,
                dadosGlobais,
                media,
                maxi,
            ...
            btn.disabled = false;
            btn.innerHTML = "📄 Gerar PDF do Dashboard";
        }
    }, 50);
}
...
function drawChartPDF(doc, vals, labels, ox, oy, cW, cH) {
    ...
    const yOf = (v) =>
        oy + padT + (1 - (v - vMin) / Math.max(1, vMax - vMin)) * gH;
    ...
    const nG = 5;
    for (let i = 0; i <= nG; i++) {
        const v = vMin + (vMax - vMin) * (i / nG);
        const y = yOf(v);
        doc.line(ox + padL, y, ox + padL + gW, y);
        doc.setTextColor(100, 120, 150);
        doc.setFontSize(5.5);
        doc.text(v.toFixed(0) + "%", ox + padL - 2, y + 1.5, {
            align: "right",
        });
    }
    for (let i = 0; i < slice.length - 1; i++) {
        const x1 = xOf(i),
            x2 = xOf(i + 1);
        const y1 = yOf(slice[i]),
            y2 = yOf(slice[i + 1]);
        const yb = yOf(vMin);
        const ytop = Math.min(y1, y2);
        doc.setFillColor(20, 40, 28);
        doc.rect(x1, ytop, x2 - x1, yb - ytop, "F");
    }
    doc.setDrawColor(74, 222, 128);
    doc.setLineWidth(0.9);
    for (let i = 0; i < slice.length - 1; i++)
        doc.line(xOf(i), yOf(slice[i]), xOf(i + 1), yOf(slice[i + 1]));
    doc.setFillColor(74, 222, 128);
    const dotStep = Math.max(1, Math.floor(slice.length / 20));
    for (let i = 0; i < slice.length; i += dotStep)
        doc.circle(xOf(i), yOf(slice[i]), 0.8, "F");
    doc.setFillColor(255, 255, 255);
    doc.circle(
        xOf(slice.length - 1),
        yOf(slice[slice.length - 1]),
        1.2,
        "F",
    );
    ...
    for (let i = 0; i < slice.length; i++) {
        if (i % xStep === 0 || i === slice.length - 1)
            doc.text(slabels[i], xOf(i), oy + cH - 1.5, { align: "center" });
    }
}
...
function buildAnalise(vals, dados, media, maxi, mini, tend) {
    ...
    let altaCount = 0,
        baixaCount = 0;
    vals.forEach((v) => {
        if (v > media + 5) altaCount++;
        if (v < media - 5) baixaCount++;
    });
    return [
        {
            titulo: "Resumo Geral",
        ...
        },
        {
            ...
            linhas: [
                media < 40
                    ? "Umidade media BAIXA — ambiente seco. Considere monitorar para evitar ressecamento."
                    : media > 70
                        ? "Umidade media ALTA — ambiente umido. Verifique ventilacao e possibilidade de mofo."
                        : "Umidade media dentro da faixa de conforto (40–70%). Condicoes normais.",
                tend === "Subindo"
                    ? "Tendencia de aumento de umidade nas ultimas leituras. Fique atento a possiveis picos."
                    : tend === "Caindo"
                        ? "Tendencia de queda de umidade. Monitorar se atingira niveis criticos baixos."
...
function addRodape(doc, W, H, pagAtual, pagTotal, agora) {
    ...
    doc.setFontSize(7);
    doc.setFont("helvetica", "bold");
    doc.text("EcoTech", 12, H - 6);
    doc.setTextColor(122, 143, 171);
    doc.setFont("helvetica", "normal");
    doc.text(`Relatorio gerado automaticamente — ${agora}`, W / 2, H - 6, {
        align: "center",
    });
    doc.text(`${pagAtual} / ${pagTotal}`, W - 12, H - 6, {
        align: "right",
    });
}
